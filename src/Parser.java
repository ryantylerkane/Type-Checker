...
{	
	public static HashMap<String, FunData> funMap = new HashMap<String, FunData>();
		// records the parameter list and body expression of each function;
		// used by Eval() to evaluate function-call expressions
	...
	
	{
		...
		if (state == State.Eq )
		{
			getToken();
			Exp exp = exp();
			FunData funData = funMap.get(header.funName);
			funData.bodyExp = exp;
	...
	
	{
		if ( state.isType() )
		{
			...
			if ( state == State.Id )
			{
				...
				funData.parameterList = parameterList;
				funMap.put(funName, funData);
